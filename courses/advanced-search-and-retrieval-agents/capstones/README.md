# Capstones — Advanced Search & Retrieval Agents

> Part of [Advanced Search & Retrieval Agents](../README.md).

Where the sibling advanced course concludes with one large tuning engagement, this course concludes with **three small capstones** — each a 5–10 day deliverable against a distinct customer scenario shape.

A partner pursuing the AS&RA Specialist cert ships **one** of the three. A partner pursuing the deep specialism (and the senior-band listing in the partner directory) ships **all three**.

## The three capstones

| | Capstone | Customer shape | Effort | Builds leveraged |
|---|---|---|---|---|
| **A** | [Multilingual Conversational Retrieval Agent](capstone-a-multilingual-conversational-agent/) | Consumer brand with multilingual customers; conversational follow-ups across language switches | 5–7 days | Builds 2, 5, 7 |
| **B** | [Cross-KB Federated Search](capstone-b-cross-kb-federated-search/) | Enterprise with multiple KBs (per business unit / region / security boundary); single search UX with per-KB attribution | 7–10 days | Builds 1, 3, 6 |
| **C** | [Query-Decomposition Compliance Agent](capstone-c-query-decomposition-compliance-agent/) | Regulated industry (financial services, healthcare, energy, public sector); multi-part compliance questions | 8–10 days | Builds 4, 7, 8 |

Pick the capstone whose customer shape most closely matches the partner's book of business.

## Capstone shape

Each capstone brief follows the same structure:

1. **Customer shape** — who is the buyer, what is the failure they're feeling today.
2. **Deliverable** — the artefact the partner ships.
3. **Architecture** — KB(s), labelsets, agents, search profiles, observability layer.
4. **Scope (in / out)** — explicit deliverables and explicit non-deliverables.
5. **Demo script** — the 15–20 minute customer demo the partner runs at the end.
6. **Pass rubric** — the criteria the Progress Solution lead reviews against.
7. **Effort breakdown** — week-by-week activity guide.
8. **Reskinning notes** — how to swap the persona for a different vertical without rebuilding from zero.

## Cert criteria

Capstone submission counts toward the AS&RA Specialist cert when:

- The artefact is deployed against a real (sandbox or customer) KB.
- The deliverables in the capstone's brief are all green.
- The demo script is rehearsed live with a Progress Solution lead in a 90-minute defence.
- The partner submits the capstone workspace folder (everything under the capstone's directory) to the partner workspace as evidence.

## See also

- Parent course: [Advanced Search & Retrieval Agents](../README.md)
- Build 8 (the immediate prerequisite for Capstone C): [Build 8 — Agent Decomposition, Tool Use & Cost Observability](../builds/build-8-agent-decomposition-and-observability/)
