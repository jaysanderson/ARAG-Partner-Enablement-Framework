...
- `content_type/spec_sheet/` (2 PDFs) — `skyline-45l-spec-sheet.pdf` (4 pages, 3 images) and `tour-du-mont-blanc-topo-guide.pdf` (5 pages, 5 images), with real embedded diagrams, product illustrations, and topographic relief maps. Used in Build 04 (`visual-rag-and-images`) to demonstrate the `page_image` and `paragraph_image` RAG image strategies — strategies that need genuine multi-page image content to produce a visible difference, which is why they're PDFs rather than markdown.
  - **Two structural constraints Build 04 depends on — don't edit these away.** (1) Page 4 of the topo guide carries **two** images (Figure 4 and Figure 5); it is the only multi-image page in either PDF, and Build 04 Step 3's `page_image`-vs-`paragraph_image` contrast is unobservable without it. (2) Several facts live *only* in the images and must never be added to the prose: the Skyline diagram's callouts 6 (stretch-mesh water-bottle pocket) and 7 (hydration reservoir port), and the topo guide's *Trient* / *Tre-le-Champ* waypoints and ~2,500 m peak height. Regenerate the topo guide with `tools/corpus/build-tmb-topo-guide.py`, which preserves both constraints.
