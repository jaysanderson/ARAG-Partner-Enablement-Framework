...
```json
{
  ...
  "prompt": {
    "system": "You are the demo AI. Maximum 3 sentences. End with one call-to-action link from the context.",
...
The system prompt is the LLM's persona, length budget, format rules, and behaviour constraints.  has the canonical "prospect-mode" system prompt:

> *"You are the demo AI. STRICT RULES: (1) Maximum 3 sentences for your answer. (2) Then provide ONE call-to-action link from the context on a new line. (3) STOP after the link. Do not continue writing. Do not repeat information. Do not add additional paragraphs after the call-to-action."*
...
Sometimes you don't need a custom prompt at all. You need to prefix the user's query with one phrase. Three examples from :
...
**Resource scoping** ():
> `` `Regarding the demo resource titled "${resourceTitle}": ${rawQuery}` ``
...
The flagship Tier 2 demo is the two-voice floating chat.  The mechanic:
...
The prospect-mode prompt references "CallToAction fields." These aren't a Nuclia feature — they're a **partner-engineered field design** that  pioneered.
...
`formatAssistantHtml` at  does the front-end intercept. It also *truncates* anything the model writes after the CTA — because "STOP after the link" in the system prompt isn't always honoured, but post-processing always wins.
...
has a hard-coded list — `English`, `Spanish`, `French`, `German`, `Mandarin`, `Japanese`. Add or remove as the customer requires.
...
```typescript
const contextQuery = `Regarding the demo resource titled "${resourceTitle}": ${rawQuery.trim()}`;
```
...
1. **Switching KBs based on user state instead of switching prompts.** Some partners (and the original ) use a two-KB pattern for public-vs-member content. That's a *gating* decision, not a *voice* decision. For Tier 2 multi-surface chat over a *single* corpus, switch prompts, not KBs.
...
Forked from 's, customised against your Build 0 KB.
